...
single and batch calls per key. If a joined load omits a key, the caller releases
all claims from that attempt, discards its staged values, and retries every
still-missing key together. This can invoke the loader again for a key returned
by an earlier discarded round, but prevents cross-round ownership deadlocks.
Loader errors, panics, or cancellation do not publish staged batch values. The
default TTL is five minutes, cleanup runs every minute, and expiration receives
up to one minute of random jitter.
